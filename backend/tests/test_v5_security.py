...
async def test_rbac_scope_protection(client: AsyncClient):
    ...
    await client.post("/api/v1/auth/register", json={
        "email": "cust_rbac@example.com",
        "password": "Password123!",
        "role": "CUSTOMER"
    })
    res_cust_login = await client.post("/api/v1/auth/login", json={
        "email": "cust_rbac@example.com",
        "password": "Password123!"
    })
    ...
    await client.post("/api/v1/auth/register", json={
        "email": "mgr_rbac@example.com",
        "password": "Password123!",
        "role": "STORE_MANAGER"
    })
    res_mgr_login = await client.post("/api/v1/auth/login", json={
        "email": "mgr_rbac@example.com",
        "password": "Password123!"
    })
