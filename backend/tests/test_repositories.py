...
async def test_product_repository_pagination_and_sorting(db_session: AsyncSession):
    await ProductRepository.create(
        db_session,
        ProductCreate(name="Alpha Phone", description="Desc A", price=Decimal("500.00"), stock=10),
    )
    await ProductRepository.create(
        db_session,
        ProductCreate(name="Beta Laptop", description="Desc B", price=Decimal("1200.00"), stock=5),
    )
    await ProductRepository.create(
        db_session,
        ProductCreate(
            name="Gamma Monitor", description="Desc C", price=Decimal("300.00"), stock=15
        ),
    )

    # Test sorting by price asc
    products, total = await ProductRepository.list_products(
        db_session, page=1, page_size=2, sort_by="price", sort_order="asc"
    )
    assert total == 3
    ...
    filtered, f_total = await ProductRepository.list_products(
        db_session, min_price=Decimal("400.00"), max_price=Decimal("1000.00")
    )
    assert f_total == 1
