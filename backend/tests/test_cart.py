...
async def test_add_item_and_get_cart(client: AsyncClient):
    ...
    prod_res = await client.post("/api/v1/products", json={"name": "Mouse", "price": 25.0, "stock": 10})
    prod_id = prod_res.json()["id"]

    add_res = await client.post("/api/v1/cart/items", headers=headers, json={"product_id": prod_id, "quantity": 2})
...
async def test_update_item_quantity(client: AsyncClient):
    ...
    prod_res = await client.post("/api/v1/products", json={"name": "Keyboard", "price": 100.0, "stock": 5})
    prod_id = prod_res.json()["id"]

    add_res = await client.post("/api/v1/cart/items", headers=headers, json={"product_id": prod_id, "quantity": 1})
    item_id = add_res.json()["cart_items"][0]["id"]

    update_res = await client.patch(f"/api/v1/cart/items/{item_id}", headers=headers, json={"quantity": 3})
...
async def test_remove_cart_item(client: AsyncClient):
    ...
    prod_res = await client.post("/api/v1/products", json={"name": "Pad", "price": 15.0, "stock": 5})
    prod_id = prod_res.json()["id"]
...
async def test_stock_validation(client: AsyncClient):
    ...
    prod_res = await client.post("/api/v1/products", json={"name": "Rare Item", "price": 500.0, "stock": 2})
    prod_id = prod_res.json()["id"]

    # Try adding more than available stock
    fail_res = await client.post("/api/v1/cart/items", headers=headers, json={"product_id": prod_id, "quantity": 5})
...
async def test_cart_ownership_validation(client: AsyncClient):
    ...
    prod_res = await client.post("/api/v1/products", json={"name": "Shared Prod", "price": 10.0, "stock": 10})
    prod_id = prod_res.json()["id"]

    # User 1 adds item
    u1_cart = await client.post("/api/v1/cart/items", headers=headers_user1, json={"product_id": prod_id, "quantity": 1})
    item_id = u1_cart.json()["cart_items"][0]["id"]

    # User 2 attempts to modify User 1's cart item
    u2_hack = await client.patch(f"/api/v1/cart/items/{item_id}", headers=headers_user2, json={"quantity": 2})
