...
async def test_keyset_pagination_and_soft_delete(client: AsyncClient):
    ...
    await client.post("/api/v1/auth/register", json={"email": "data_mgr@example.com", "password": "Password123!", "role": "STORE_MANAGER"})
    login_res = await client.post("/api/v1/auth/login", json={"email": "data_mgr@example.com", "password": "Password123!"})
    token = login_res.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}

    # Create 3 products
    p1 = (await client.post("/api/v1/products", json={"name": "Item A", "price": 10.0, "stock": 5}, headers=headers)).json()
    p2 = (await client.post("/api/v1/products", json={"name": "Item B", "price": 20.0, "stock": 5}, headers=headers)).json()
    p3 = (await client.post("/api/v1/products", json={"name": "Item C", "price": 30.0, "stock": 5}, headers=headers)).json()
