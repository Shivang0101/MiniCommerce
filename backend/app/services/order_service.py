...
class OrderService:
    ...
    async def create_order_checkout(
        ...
        try:
            ...
            redis = get_redis()
            if redis:
                await redis.setex(f"order_status:{order.id}", 3600, json.dumps({
                    "status": "Confirmed",
            ...
            spans = [
                {"service": "FastAPI", "name": "POST /api/v1/orders/checkout", "duration_ms": total_checkout_ms},
                {"service": "PostgreSQL", "name": "SELECT FOR UPDATE (Locking)", "duration_ms": select_lock_ms},
                {"service": "PostgreSQL", "name": "INSERT order & order_items", "duration_ms": insert_order_ms},
                {"service": "Redis ARQ", "name": "Enqueue Worker Tasks", "duration_ms": arq_enqueue_ms},
            ]
