...
async def send_receipt_email(ctx: dict[str, Any], order_id: str, user_email: str, total_amount: str) -> dict[str, Any]:
    ...
    if redis:
        ...
        await redis.setex(f"order_status:{order_id}", 3600, json.dumps({
...
async def audit_low_stock(ctx: dict[str, Any], product_items: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for item in product_items:
        ...
        if remaining_stock <= 5:
            ...
            if redis:
                await redis.setex(f"low_stock_alert:{prod_id}", 86400, json.dumps({
                    "product_id": prod_id,
...
async def record_analytics_event(ctx: dict[str, Any], event_type: str, payload: dict[str, Any]) -> dict[str, Any]:
    redis = ctx.get("redis")
    if redis:
        try:
