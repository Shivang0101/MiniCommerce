...
async def refresh(request: Request, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    if jti:
        await TokenBlacklistService.revoke_token(jti, ttl_seconds=settings.REFRESH_TOKEN_EXPIRE_DAYS * 86400)
...
async def logout(response: Response, request: Request, current_user: User = Depends(get_current_user)):
    ...
    if auth_header and auth_header.startswith("Bearer "):
        ...
        if payload and payload.get("jti"):
            await TokenBlacklistService.revoke_token(payload["jti"], ttl_seconds=settings.ACCESS_TOKEN_EXPIRE_MINUTES * 60)

    refresh_token = request.cookies.get(settings.REFRESH_COOKIE_NAME)
    if refresh_token:
        r_payload = decode_access_token(refresh_token)
        if r_payload and r_payload.get("jti"):
            await TokenBlacklistService.revoke_token(r_payload["jti"], ttl_seconds=settings.REFRESH_TOKEN_EXPIRE_DAYS * 86400)
